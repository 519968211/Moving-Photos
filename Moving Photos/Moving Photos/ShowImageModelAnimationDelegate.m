...
@implementation ShowImageModelAnimationDelegate
...
- (void)presentViewAnimation:(id <UIViewControllerContextTransitioning>)transitionContext {
    ...
    destinationView.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height);
    // 将目标view添加到容器view
    ...
    _photoAnimateImageView = [[UIImageView alloc] initWithFrame:_dissmissRect];
    _photoAnimateImageView.image = _image;
    [_photoAnimateView addSubview:_photoAnimateImageView];
    
    // 执行过渡动画
    __weak ShowImageModelAnimationDelegate *weakSelf = self;
    [UIView animateWithDuration:1.0 animations:^{
        weakSelf.photoAnimateImageView.frame = CGRectMake(0, ([[UIScreen mainScreen] bounds].size.height-height)/2.0, [[UIScreen mainScreen] bounds].size.width, height);
